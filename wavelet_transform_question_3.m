% 3. Write a MATLAB program to implement to show filtering of images in transform domain using
% two-dimensional Transform.

clc;
img = imread('Monuments_1002.jpg');
figure,imshow(img);
title("original image");
myimg = imresize(img,[256 ,256]);
m = myimg(:,:,3);
noise = imnoise(m,"salt & pepper",0.02);
figure,imshow(noise);
title("noise in  image");


[ca,ch,cv,cd] = dwt2(noise,'haar');
